...
function renderGallery(){
  ...
  doodles.forEach((d,i)=>{
    ...
    numEl.textContent=String(i+1).padStart(3,'0');
    ...
    thumbDiv.innerHTML=resizeSVG(d.svg, VSIZE, VSIZE);
    ...
    item.addEventListener('click',()=>loadForEditing(d));

    const del=document.createElement('button');
    del.className='gallery-delete';del.textContent='×';
    del.addEventListener('click',e=>{
      e.stopPropagation();
      const list=loadDoodles();list.splice(i,1);
      localStorage.setItem(DOODLE_KEY,JSON.stringify(list));
