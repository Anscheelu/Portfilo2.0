...
function applyBgColor(val){
  bgColor = val;
  canvasBg.style.background = val;
}
const _bgPicker = document.getElementById('bgColorPicker');
_bgPicker.addEventListener('input',  e => applyBgColor(e.target.value));
_bgPicker.addEventListener('change', e => applyBgColor(e.target.value));
document.getElementById('bgResetBtn').addEventListener('click', () => {
  _bgPicker.value = '#f6f1e7';
  applyBgColor('#f6f1e7');
});
...
function renderGallery(){
  ...
  [...doodles].reverse().forEach((d,i)=>{
    ...
    // For video items, use full canvas view so thumb matches the video framing
    thumbDiv.innerHTML = hasVideo ? resizeSVG(d.svg) : fitToContent(d.svg);
    ...
    if (hasVideo) {
      ...
      const layerSVGs=(d.savedLayers||[]).map(ld=>{
        ...
        const rawStr=new XMLSerializer().serializeToString(root);
        const fitted=fitToContent(rawStr);
        return 'data:image/svg+xml;charset=utf-8,'+encodeURIComponent(fitted);
      }).filter(Boolean);
...
function renderStartBg(){
  ...
  // Smaller shapes on small screens, closer to title
  const isMobile = vw <= 640;
  const SIZE = isMobile ? 110 : 180;
  const GAP  = isMobile ? 10 : 16;
  const TEXT_PAD = isMobile ? 24 : 50;
  ...
  // Randomly pick a subset of doodles for the background
  const shuffled = [...doodles].sort(() => Math.random() - 0.5);
  const maxShow = Math.min(shuffled.length, isMobile ? 6 : 12);
  const selected = shuffled.slice(0, maxShow);
